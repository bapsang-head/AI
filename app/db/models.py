...
class NutritionInfo(Base):
    ...
    food = Column(String(50), unique=True, nullable=False)  # 음식 이름 
    calories = Column(Integer, nullable=False)  # 칼로리 정보 
    protein = Column(Float, nullable=False)  # 단백질 정보 
    fat = Column(Float, nullable=False)  # 지방 정보 
    carbohydrates = Column(Float, nullable=False)  # 탄수화물 정보 

# UnitConversion 클래스는 특정 음식의 단위 변환 정보를 저장합니다.
class UnitConversion(Base):
    __tablename__ = 'unit_conversion'  # 데이터베이스에서 사용할 테이블 이름
    id = Column(Integer, primary_key=True)  # 기본 키 열
    food = Column(String, nullable=False)  # 음식 이름 
    unit = Column(String, nullable=False)  # 단위 
    grams = Column(Float, nullable=False)  # 그램으로 변환된 값
