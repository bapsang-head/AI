...
class FancyNerProcessor(object):
    """Processor for the Food NER data set """
    ...
    def _create_examples(self, dataset, set_type):
        ...
        for line in dataset:
            if line.strip() == "":
                if words and labels:
                    guid = "%s-%s" % (set_type, guid_index)
                    labels_idx = [self.labels_lst.index(label) if label in self.labels_lst else self.labels_lst.index("UNK") for label in labels]
        ...
        # 마지막 예제가 끝난 후에 처리되지 않은 예제가 있으면 추가
        if words and labels:
            guid = "%s-%s" % (set_type, guid_index)
            labels_idx = [self.labels_lst.index(label) if label in self.labels_lst else self.labels_lst.index("UNK") for label in labels]
            assert len(words) == len(labels_idx)
